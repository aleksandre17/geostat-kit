...
case "$CMD" in
  ...
  stack)
    exec bash "$PKG/toolkit/stack/compose.sh" "$@"
    ;;
  layout)
    exec powershell -ExecutionPolicy Bypass -File "$PKG/toolkit/layout/simulate-server-layout.ps1" "$@"
    ;;
  mod|fe|be|help)
    ...
    ;;
  *) exec powershell -ExecutionPolicy Bypass -File "$ROOT/tools/geostat.ps1" "$CMD" "$@" ;;
esac
